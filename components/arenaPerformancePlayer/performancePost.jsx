...
import CommentDrawer from "./comment/commentDrawer";
  ...
  export default function PerformancePost({
    ...
    const { scale, setGlobalArenaRefresh } = useGlobalContext()
   ...
   const noSpotLight = isLocalSpotlight  || isRegionalSpotlight ||  isGlobalSpotlight 
    ...
    return (
      ...
      >
        ...
        <PostData item={item} width={width} onPress = {loadComments} setOpenCommentDrawer = {setOpenCommentDrawer} arena={selectedArena} post = {item}
        ...
        {noSpotLight && paused && (
          <View className="absolute top-2 right-2  flex-row items-center gap-1 rounded-full bg-black/70  ">
            <SpotlightIcon size ={scale(18)} />
          </View>
        )}
        {paused && (
        <PostFooter post = {item} width = {width} loadProfile = {loadProfile} isFollower = {isFollower} />
        )}
